...
class MercadopagoConfig
{
    ...
    public function crearTablaCustomTransacciones()
    {
    ...
    public function eliminarTablaCustomTransacciones()
    {
        $nombreTabla = 'bapp_mercadopago';
        if (WHMCS\Database\Capsule::schema()->hasTable($nombreTabla)) {
            try {
    ...
    public function getPreferenciaPago($accesstoken, $datos_mp, $prueba = false)
    {
        $userid = substr(strrchr($accesstoken, '-'), 1);
        $uri    = 'https://api.mercadopago.com/checkout/preferences/';
        $data   = [
            'additional_info' => '',
            'auto_return'     => $datos_mp['retorno'],
            'back_urls'       => [
                'failure' => $datos_mp['url_fallo'],
                'pending' => $datos_mp['url_pendiente'],
                'success' => $datos_mp['url_exito']
            ],
            'binary_mode'         => true,
            'merchant_account_id' => $datos_mp['merchant_account_id'],
            'processing_modes'    => [$datos_mp['processing']],
            'processing_mode'     => $datos_mp['processing'],
            'external_reference'  => $datos_mp['referencia'],
            'items'               => [
                [
                    'id'          => '',
                    'currency_id' => $datos_mp['item_moneda'],
                    'title'       => $datos_mp['item_titulo'],
                    'picture_url' => $datos_mp['item_imagen'],
                    'description' => $datos_mp['item_descripcion'],
                    'category_id' => 'services',
                    'quantity'    => 1,
                    'unit_price'  => $datos_mp['item_precio']
                ]
            ],
            'notification_url' => $datos_mp['notification_url'],
            'payer'            => [
                'phone' => [
                    'area_code' => $datos_mp['comprador_telefono_codigodearea'],
                    'number'    => $datos_mp['comprador_telefono_numero']],
                'address' => [
                    'zip_code'      => $datos_mp['comprador_domicilio_codigopostal'],
                    'street_name'   => $datos_mp['comprador_domicilio_calle'],
                    'street_number' => $datos_mp['comprador_domicilio_numero']], '
                            identification' => [
                        'number' => $datos_mp['comprador_documento_numero'],
                        'type'   => $datos_mp['comprador_documento_tipo']
                    ],
                'email'   => $datos_mp['comprador_email'],
                'name'    => $datos_mp['comprador_nombre'],
                'surname' => $datos_mp['comprador_apellido']
            ],
            'payment_methods' => [
                'excluded_payment_types' => $datos_mp['exclusiones']
            ]
        ];
    ...
    public function getConfigModulo()
    {
        $modulo       = $this->modulo;
        ...
        $configHeader = [
            'bh_xxx' => [
                'FriendlyName' => traduccion($idioma, 'mpconfig_2'),
                'Description'  => "<br><br><b><a href='https://github.com/fedealvz/WHMCS-MercadoPago' target='_blank'>https://github.com/fedealvz/WHMCS-MercadoPago</a></b><br><br><br>"
            ],
            'FriendlyName' => [
                'Type'  => 'System',
                'Value' => $nombre
            ...
            'bh_Access_Token' => [
                'FriendlyName' => '<b>Access Token</b>:',
                'Type'         => 'text',
                'Size'         => '150',
                'Description'  => "&nbsp;<a href='#' onclick='\$(\"#Client_id\").modal(\"show\");' class='btn btn-warning btn-xs'>" . traduccion($idioma, 'mpconfig_4') . "</a><br>\r\n            " . traduccion($idioma, 'mpconfig_5') . ":\r\n            <br><a href='https://www.mercadopago.com/mla/account/credentials' target='_blank' class='btn btn-info btn-xs'>Argentina</a>\r\n            &nbsp;<a href='https://www.mercadopago.com/mlb/account/credentials' target='_blank' class='btn btn-info btn-xs'>Brasil</a>\r\n            &nbsp;<a href='https://www.mercadopago.com/mlc/account/credentials' target='_blank' class='btn btn-info btn-xs'>Chile</a>\r\n            &nbsp;<a href='https://www.mercadopago.com/mco/account/credentials' target='_blank' class='btn btn-info btn-xs'>Colombia</a>\r\n            &nbsp;<a href='https://www.mercadopago.com/mlm/account/credentials' target='_blank' class='btn btn-info btn-xs'>México</a>\r\n            &nbsp;<a href='https://www.mercadopago.com/mpe/account/credentials' target='_blank' class='btn btn-info btn-xs'>Perú</a>\r\n            &nbsp;<a href='https://www.mercadopago.com/mlu/account/credentials' target='_blank' class='btn btn-info btn-xs'>Uruguay</a>\r\n            &nbsp;<a href='https://www.mercadopago.com/mlv/account/credentials' target='_blank' class='btn btn-info btn-xs'>Venezuela</a>\r\n\r\n\r\n\r\n            <div class='modal fade' id='Client_id' tabindex='-1' role='dialog' aria-labelledby='DeactivateGatewayLabel' aria-hidden='true'>\r\n                <div class='modal-dialog'>\r\n                    <div class='modal-content panel panel-primary'>\r\n                        <div id='modalDeactivateGatewayHeading' class='modal-header panel-heading'>\r\n                            <button type='button' class='close' data-dismiss='modal'> <span aria-hidden='true'>&times;</span> <span class='sr-only'>" . traduccion($idioma, 'mpconfig_6') . "</span> </button>\r\n                            <h4 class='modal-title' id='DeactivateGatewayLabel'>" . traduccion($idioma, 'mpconfig_4') . ": Access Token</h4>\r\n                        </div>\r\n                        <div id='modalDeactivateGatewayBody' class='modal-body panel-body'>\r\n                            <p>" . traduccion($idioma, 'mpconfig_7') . "\r\n                            <br><span style='color:red'>" . traduccion($idioma, 'mpconfig_8') . "</span></p>\r\n                        </div>\r\n                        <div id='modalDeactivateGatewayFooter' class='modal-footer panel-footer'>\r\n                            <button type='button' id='DeactivateGateway-Cancelar' class='btn btn-default' data-dismiss='modal'> " . traduccion($idioma, 'mpconfig_6') . " </button>\r\n                        </div>\r\n                    </div>\r\n                </div>\r\n            </div>\t\r\n            "
            ],
            'merchant_account_id' => [
                'FriendlyName' => 'Merchant Account ID:',
                'Type'         => 'text',
                'Size'         => '100',
                'Description'  => '<br>' . traduccion($idioma, 'mpconfig_67')
            ],
            'processing' => [
                'FriendlyName' => 'Processing Mode:',
                'Type'         => 'dropdown',
                'Options'      => ['aggregator' => 'Aggregator (Default)', 'gateway' => 'Gateway'],
                'Description'  => traduccion($idioma, 'mpconfig_68')
            ],
            'useradmin' => [
                'FriendlyName' => 'UserName:',
                'Type'         => 'text',
                'Size'         => '100',
                'Description'  => '<br>' . traduccion($idioma, 'mpconfig_72')
            ],
            'bh_success' => [
                'FriendlyName' => traduccion($idioma, 'mpconfig_9') . ':',
                'Type'         => 'text',
                'Size'         => '100',
                'Description'  => '<br>' . traduccion($idioma, 'mpconfig_12')
            ],
            'bh_pending' => [
                'FriendlyName' => traduccion($idioma, 'mpconfig_10') . ':',
                'Type'         => 'text',
                'Size'         => '100',
                'Description'  => '<br>' . traduccion($idioma, 'mpconfig_12')
            ],
            'bh_failure' => [
                'FriendlyName' => traduccion($idioma, 'mpconfig_11') . ':',
                'Type'         => 'text',
                'Size'         => '100',
                'Description'  => '<br>' . traduccion($idioma, 'mpconfig_12')
            ],
            'bh_titulo' => [
                'FriendlyName' => "<span style='color:red'>" . traduccion($idioma, 'mpconfig_13') . '</span>:',
                'Type'         => 'text',
                'Size'         => '50',
                'Value'        => 'Factura',
                'Description'  => ' ' . traduccion($idioma, 'mpconfig_14')
            ],
            'bh_texto' => [
                'FriendlyName' => "<span style='color:red'>" . traduccion($idioma, 'mpconfig_15') . '</span>:',
                'Type'         => 'text',
                'Value'        => traduccion($idioma, 'mpconfig_16')
            ],
            'color' => [
                'FriendlyName' => traduccion($idioma, 'mpconfig_17') . ':',
                'Type'         => 'dropdown',
                'Options'      => [
                    'primary'   => traduccion($idioma, 'mpconfig_18'),
                    'secondary' => traduccion($idioma, 'mpconfig_19'),
                    'success'   => traduccion($idioma, 'mpconfig_20'),
                    'danger'    => traduccion($idioma, 'mpconfig_21'),
                    'warning'   => traduccion($idioma, 'mpconfig_22'),
                    'info'      => traduccion($idioma, 'mpconfig_23'),
                    'light'     => traduccion($idioma, 'mpconfig_24'),
                    'dark'      => traduccion($idioma, 'mpconfig_25'),
                    'link'      => traduccion($idioma, 'mpconfig_26')
                ],
                'Description' => traduccion($idioma, 'mpconfig_27')
            ],
            'bh_nota' => [
                'FriendlyName' => traduccion($idioma, 'mpconfig_28') . ':',
                'Type'         => 'text',
                'Size'         => '100',
                'Description'  => '<br>' . traduccion($idioma, 'mpconfig_29')
            ],
            'bh_credit_card' => [
                'FriendlyName' => traduccion($idioma, 'mpconfig_30') . ':',
                'Type'         => 'yesno',
                'Description'  => traduccion($idioma, 'mpconfig_31')
            ],
            'bh_ticket' => [
                'FriendlyName' => traduccion($idioma, 'mpconfig_32') . ':',
                'Type'         => 'yesno',
                'Description'  => traduccion($idioma, 'mpconfig_33')
            ],
            'bh_atm' => [
                'FriendlyName' => traduccion($idioma, 'mpconfig_34') . ':',
                'Type'         => 'yesno',
                'Description'  => traduccion($idioma, 'mpconfig_35')
            ],
            'bh_debito' => [
                'FriendlyName' => traduccion($idioma, 'mpconfig_36') . ':',
                'Type'         => 'yesno',
                'Description'  => traduccion($idioma, 'mpconfig_37')
            ],
            'bh_prepaga' => [
                'FriendlyName' => traduccion($idioma, 'mpconfig_38') . ':',
                'Type'         => 'yesno',
                'Description'  => traduccion($idioma, 'mpconfig_39')
            ],
            'bh_banco' => [
                'FriendlyName' => traduccion($idioma, 'mpconfig_40') . ':',
                'Type'         => 'yesno',
                'Description'  => traduccion($idioma, 'mpconfig_41')
            ],
            'bh_comportamiento' => [
                'FriendlyName' => traduccion($idioma, 'mpconfig_42') . ':',
                'Type'         => 'dropdown',
                'Options'      => [
                    'normal'     => traduccion($idioma, 'mpconfig_43'),
                    'noverifica' => traduccion($idioma, 'mpconfig_44'),
                    'truncado'   => traduccion($idioma, 'mpconfig_45'),
                    'redondeado' => traduccion($idioma, 'mpconfig_46')
                ],
                'Description' => traduccion($idioma, 'mpconfig_47') . "<br>\r\n            <b>" . traduccion($idioma, 'mpconfig_43') . '</b>: ' . traduccion($idioma, 'mpconfig_48') . "\r\n            <br><b>" . traduccion($idioma, 'mpconfig_44') . '</b>: ' . traduccion($idioma, 'mpconfig_49') . "\r\n            <br><b>" . traduccion($idioma, 'mpconfig_45') . '</b>: ' . traduccion($idioma, 'mpconfig_50') . "\r\n            <br><b>" . traduccion($idioma, 'mpconfig_46') . '</b>: ' . traduccion($idioma, 'mpconfig_51')
            ],
            'bh_error_mp' => [
                'FriendlyName' => traduccion($idioma, 'mpconfig_52') . ':',
                'Type'         => 'yesno',
                'Description'  => traduccion($idioma, 'mpconfig_53')
            ],
            'prueba' => [
                'FriendlyName' => traduccion($idioma, 'mpconfig_54') . ':',
                'Type'         => 'yesno',
                'Description'  => traduccion($idioma, 'mpconfig_55')
            ],
            'email' => [
                'FriendlyName' => traduccion($idioma, 'mpconfig_70') . ':',
                'Type'         => 'text',
                'Size'         => '100',
                'Description'  => '<br>' . traduccion($idioma, 'mpconfig_71')
            ]
        ];
        return $configHeader;
    }

    public function getLinkPago($params)
    {
        ...
        if ($bh_credit_card == 'on') {
            $mediosno[] = ['id' => 'credit_card'];
        }
        if ($bh_ticket == 'on') {
            $mediosno[] = ['id' => 'ticket'];
        }
        if ($bh_banco == 'on') {
            $mediosno[] = ['id' => 'bank_transfer'];
        }
        if ($bh_atm == 'on') {
            $mediosno[] = ['id' => 'atm'];
        }
        if ($bh_debito == 'on') {
            $mediosno[] = ['id' => 'debit_card'];
        }
        if ($bh_prepaga == 'on') {
            $mediosno[] = ['id' => 'prepaid_card'];
        }
    ...
    public function mercadopagoIPN($gatewayOBJ)
    {
        ...
        $postData          = ['transid' => $informe_cobro];
        $arr_transacciones = localAPI($command, $postData, $adminUsername);
        if ($arr_transacciones['totalresults'] == 0) {
            Capsule::table('bapp_mercadopago')->insert([
                'transaccion' => $informe_cobro,
    ...
    public function getPaymentMercadopago($transaccion, $accessToken)
    {
    ...
    public function callbackMercadopago($idtrans = '')
    {
        if (!empty($idtrans)) {
            ...
            if ($arr_transacciones['totalresults'] == 0) {
                ...
                if ($status == 'approved') {
                    ...
                    if (!empty($nrofactura)) {
                        ...
                        $postData  = ['paymentmethod' => $GATEWAY['paymentmethod'], 'invoiceid' => $nrofactura, 'transid' => $mp_transaccion, 'amountin' => $importe_pagado, 'fees' => $comision];
                    ...
                    if ($status == 'pending') {
                        ...
                        $postData  = ['invoiceid' => $datosdelpago['external_reference'], 'notes' => date('Y-m-d H:i:s') . ': ' . traduccion($idioma, 'mpconfig_76') . "\r\n                        [" . ucwords($datosdelpago['payment_type_id']) . ' - ' . ucwords($datosdelpago['payment_method_id']) . ']'];
                        $results   = localAPI($command, $postData, $adminUsername);
                        ...
                        Capsule::table('tbltransaction_history')->insert(['invoice_id' => $datosdelpago['external_reference'], 'gateway' => $GATEWAY['name'], 'updated_at' => date('Y-m-d H:i:s'), 'transaction_id' => $mp_transaccion, 'remote_status' => traduccion($idioma, 'mpconfig_76'), 'description' => $datosdelpago['payment_type_id'] . ' - ' . $datosdelpago['payment_method_id']]);
                    }
                    $resultado = Capsule::table('bapp_mercadopago')->where('id', '=', $mp_id)->delete();
                }
            }
